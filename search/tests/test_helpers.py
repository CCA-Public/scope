...
class FunctionsTests(TestCase):
    ...
    def test_add_query_to_search_empty_query(self):
        modified_search = add_query_to_search(
            self.search, '', self.query_fields)
    ...
    def test_add_query_to_search_populated_query(self):
        query = 'test_query'
        modified_search = add_query_to_search(
            self.search, query, self.query_fields)
        expected_search = {
            'query': {
                'simple_query_string': {
                    'query': query,
            ...
            'query': {
                'bool': {
                    'must': [
                        ...
                        {'terms':
                            {'collection.title.raw': params['collections']}},
                        {'range': {'datemodified': {'gte': params['start_date'],
                                                    'format': 'yyyy-MM-dd'}}},
                        {'range': {'datemodified': {'lte': params['end_date'],
                                                    'format': 'yyyy-MM-dd'}}}
                    ]
                }
            }
