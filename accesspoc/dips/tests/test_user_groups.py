...
class UserGroupsTests(TestCase):
    def setUp(self):
        ...
        self.editor, _ = Group.objects.get_or_create(name='Edit Collections and Folders')
    ...
    def test_user_groups_changes(self):
        ...
        data = {
            ...
            'password1': 'test123test',
            'password2': 'test123test',
            'groups': [self.editor.pk, self.group_b.pk],
        }
        self.client.post(url, data)
        created_user_2 = User.objects.filter(username='test2').get()
        self.assertTrue(created_user_2.is_editor())
        self.assertEqual(created_user_2.group_names(), 'Edit Collections and Folders, Group B')

        # Edit user adding groups
        url = reverse('edit_user', kwargs={'pk': created_user.pk})
        data = {
            'username': 'test',
            'groups': [self.editor.pk, self.group_c.pk],
        }
        self.client.post(url, data)
        created_user = User.objects.filter(username='test').get()
        self.assertTrue(created_user.is_editor())
        self.assertEqual(created_user.group_names(), 'Edit Collections and Folders, Group C')
        ...
        data = {
            'username': 'test',
            'groups': [self.group_a.pk, self.group_b.pk],
        }
        self.client.post(url, data)
        created_user = User.objects.filter(username='test').get()
        self.assertFalse(created_user.is_editor())
