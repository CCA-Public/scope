...
class DublinCoreSettingsTests(TestCase):
    def test_dc_optional_fields(self):
        optional_fields = {
            'title': _('title'), 'creator': _('creator'),
            'subject': _('subject'), 'description': _('description'),
            'publisher': _('publisher'), 'contributor': _('contributor'),
            'date': _('date'), 'type': _('type'),
            'format': _('format'), 'source': _('source'),
            'language': _('language'), 'coverage': _('coverage'),
            'rights': _('rights'),
        }
    ...
    def test_dc_settings_default_values(self):
        enabled_fields = [
            ...
            'publisher', 'contributor', 'date', 'type', 'format', 'source',
    ...
    def test_dc_settings_change(self):
        ...
        Setting.objects.filter(name='hide_empty_dc_fields').update(
            value=False
        )
